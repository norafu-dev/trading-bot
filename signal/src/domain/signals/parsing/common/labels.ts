...
/**
 * Closed set of coarse-grained labels the classifier assigns to a `MessageBundle`.
 *
 * The classifier's only job is to route the bundle to the right downstream path:
 ...
 * happens inside the extractor's `updateType` field, not here.
 *
 * This is a closed enum: the LLM is constrained to return exactly one of these
 * values. If uncertain, it should pick the closest match and report low
 * confidence rather than invent a new label.
 */
export const classificationLabelSchema = z.enum([
  ...
  'recap',             // Win/loss recap, performance summary
])

/** TypeScript type inferred from `classificationLabelSchema`. */
