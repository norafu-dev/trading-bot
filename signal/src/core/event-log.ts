 ...
 * @adapted-from reference/OpenAlice/src/core/event-log.ts
 *   Changes from original:
 *   - Default logPath changed from hardcoded string to PATHS.eventLog (DEC-009).
 *   - No other logic changes; core dual-write, ring-buffer, and subscriber
 *     patterns are identical to the OpenAlice original.
 *
...
export interface EventLogEntry<T = unknown> {
  ...
  ts: number
  /** Event type, e.g. "signal.received", "trade.executed". */
  type: string
...
export async function createEventLog(opts?: {
  ...
  const logPath = opts?.logPath ?? PATHS.eventLog
